[T4Scaffolding.ControllerScaffolder("SG Controller with read/write action and views, using EF data access code", Description = "Adds an ASP.NET MVC controller with views and data access code", SupportsModelType = $true, SupportsDataContextType = $true, SupportsViewScaffolder = $true)][CmdletBinding()]
param(     
...
$templateName = if($Repository) { "ControllerWithRepository" } else { "SGControllerWithContext" }
